...
type FederationRequest struct {
	...
	fields struct {
		...
		Signatures  map[spec.ServerName]map[xtools.KeyID]string `json:"signatures,omitempty"`
	}
}
...
func (r *FederationRequest) SetContent(content interface{}) error {
	if r.fields.Content != nil {
		return fmt.Errorf("xtools: content already set on the request")
	}
	if r.fields.Signatures != nil {
		return fmt.Errorf("xtools: the request is signed and cannot be modified")
	}
...
func (r *FederationRequest) Sign(serverName spec.ServerName, keyID xtools.KeyID, privateKey ed25519.PrivateKey) error {
	if r.fields.Origin != "" && r.fields.Origin != serverName {
		return fmt.Errorf("xtools: the request is already signed by a different server")
	}
	...
	signedData, err := xtools.SignJSON(string(serverName), keyID, privateKey, data)
	if err != nil {
		return err
	}
...
func (r *FederationRequest) HTTPRequest() (*http.Request, error) {
	...
	if httpReq.URL.RequestURI() != r.fields.RequestURI {
		return nil, fmt.Errorf(
			"xtools: Request URI didn't encode properly. Wanted %q. Got %q",
	...
	for keyID, sig := range r.fields.Signatures[r.fields.Origin] {
		...
		if !isSafeInHTTPQuotedString(string(r.fields.Origin)) {
			return nil, fmt.Errorf("xtools: Request Origin isn't safe to include in an HTTP header")
		}
		if !isSafeInHTTPQuotedString(string(keyID)) {
			return nil, fmt.Errorf("xtools: Request key ID isn't safe to include in an HTTP header")
		}
...
func VerifyHTTPRequest(
...
) (*FederationRequest, xutil.JSONResponse) {
	request, err := readHTTPRequest(req)
	if err != nil {
		xutil.GetLogger(req.Context()).WithError(err).Print("Error parsing HTTP headers")
		return nil, xutil.MessageResponse(400, "Bad Request")
	}
	if request.fields.Destination != "" {
		...
		case isLocalServerName == nil && destination != request.fields.Destination:
			...
			xutil.GetLogger(req.Context()).Warn(message)
			return nil, xutil.MessageResponse(400, message)
		}
	...
	if err != nil {
		xutil.GetLogger(req.Context()).WithError(err).Print("Error parsing JSON")
		return nil, xutil.MessageResponse(400, "Invalid JSON")
	}
	...
	results, err := keys.VerifyJSONs(req.Context(), []xtools.VerifyJSONRequest{{
	...
	if err != nil {
		...
		xutil.GetLogger(req.Context()).WithError(err).Print(message)
		return nil, xutil.MessageResponse(500, message)
	}
	if results[0].Error != nil {
		message := "Invalid request signature"
		xutil.GetLogger(req.Context()).WithError(results[0].Error).Print(message)
		return nil, xutil.MessageResponse(401, message)
	}

	return request, xutil.JSONResponse{Code: 200, JSON: struct{}{}}
}
...
func readHTTPRequest(req *http.Request) (*FederationRequest, error) { // nolint: gocyclo
	...
	if len(content) != 0 {
		...
		if err != nil {
			return nil, fmt.Errorf("xtools: The request had an invalid Content-Type header: %w", err)
		}
	...
	for _, authorization := range req.Header["Authorization"] {
		...
		if result.fields.Signatures == nil {
			result.fields.Signatures = map[spec.ServerName]map[xtools.KeyID]string{origin: {key: sig}}
		} else {
