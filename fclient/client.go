...
retryResolution:
	...
	for _, result := range resolutionResults {
		...
		xutil.GetLogger(r.Context()).Debugf("Error sending request to %s: %v",
			u.String(), err)
	}
...
func (fc *Client) LookupServerKeys(
	...
	request := struct {
		ServerKeyMap map[spec.ServerName]map[xtools.KeyID]keyreq `json:"server_keys"`
	}{map[spec.ServerName]map[xtools.KeyID]keyreq{}}
	for k, ts := range keyRequests {
		server := request.ServerKeyMap[k.ServerName]
		if server == nil {
			server = map[xtools.KeyID]keyreq{}
	...
	var res struct {
		ServerKeyList []xtools.ServerKeys
	}
...
func (fc *Client) DoHTTPRequest(ctx context.Context, req *http.Request) (*http.Response, error) {
	...
	newCtx := xutil.ContextWithLogger(ctx, logger)
	if fc.userAgent != "" {
